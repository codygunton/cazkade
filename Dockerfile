# Cascade library and original Dockerfile copyright notice:
...
# Copy Rust toolchain
COPY --from=rust /bootstrap/cargo /usr/local/cargo
COPY --from=rust /bootstrap/rustup /usr/local/rustup
...
## Some environment variables used by Cascade
...
WORKDIR /workspace
COPY init_corpus.sh generate.sh fuzz.sh .crashignore ./
RUN chmod +x generate.sh fuzz.sh init_corpus.sh
